...
eligibility logic (>25 cross-border payments per payee per quarter), renders
PaymentData XML, and validates it using the official CESOP Validation Module.

## Highlights
- End-to-end flow: synthetic CSV -> CESOP XML -> validation result.
- Cross-border and threshold logic aligned to the CESOP reporting trigger.
...
- Grouping by payee and quarter with >25 rule.
...
## Quickstart
Generate CSV, render XML, validate:
```sh
...
cargo run -- validate --input data/output/cesop_2025_Q4.xml --output data/output/validation.xml
```
...
```sh
cargo run -- validate \
  --input data/output/cesop_2025_Q4.xml \
